...
1. **反问 gate（R3.5 硬约束 · 弱模型鲁棒性）**：用户选定调性后（或非前端项目跳过 0.6 后），用结构化提问把"为什么 / 给谁 / 解决什么 / 何时算完"问清楚。每轮最多 3 个问题（避免啰嗦，AC-7），等用户回答再继续。**gate 规则：反问未完成（用户未回答）前，禁止产出 CHANGE.md / 进入下一步**——弱模型最易跳反问直接出方案，此为硬 gate。CHANGE.md 关键字段（Why / What / 范围排除 / 验收线）采用填空式，未填全则产出不全。
   - **不允许与调性卡片同屏呈现**（以免信息超载）
2. **影响面判定**：
...
jq --arg ts "$(date -Iseconds)" \
  '.goal.current_phase = "1" | .phase = "1" | .goal.phases_done += ["0"] | .goal.gates["0→1"] = "passed" | .updated_at = $ts' \
