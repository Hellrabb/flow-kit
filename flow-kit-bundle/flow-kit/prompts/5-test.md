...
> **检测**：`.flow-active.goal.gate_config["5-test"]` ∈ {`L2`,`both`}（`independent`/`true` 向后兼容映射为 `both`），或运行时 stop-hook.json（dsh：`.flow-kit/stop-hook.json`；claude/opencode：`.claude/stop-hook.json`）的 `independent_review.phases` 含 `"5-test"`。未开启 → 跳过本段，直接进「阶段完成自检」。
