...
> **来源**：`flow-kit-bundle/flow-kit/prompts/independent/L2-blind-review.md`（唯一维护源）
> **同步要求（L-031 锚点登记）**：本段是 L2-blind-review.md 的**全文拷贝**，必须与源文件保持一致。源文件变更时本段须同步更新（源文件是固化指令唯一维护源，本段是分发副本）。禁止凭记忆重写本段。
