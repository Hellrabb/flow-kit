...
## 弱模型鲁棒性原则（protect the weakest）

> 完整规则见 `@flow-kit/RULES.md` R3.5 / R6.1 / R7.4；哲学决策见 `@.specs/adr/001-protect-the-weakest.md`。

flow-kit 的规则/prompt 默认按"最弱模型能扛住"的标准写——所有人开局即受保护，无需 opt-in。**只加结构刚性护栏**（禁跳反问 R3.5 / 引用前证据链 R6.1 / 动手前复述边界 R7.4 / 填空式 gate / 关键节点 checkpoint / 阶段入场 goal 锚定），**不加重复唠叨**（每步复述、每操作 checkpoint、冗余自检会反噬强模型、甚至增幻觉）。

- 弱模型（易幻觉/易跳步）：靠结构与工具兜底，不靠自觉
- 强模型：上述结构刚性约束本就是好习惯，不啰嗦
- 否决"运行时模型能力自动探测"（不可靠）；降级 opt-out（`model_tier: strong`）留 L4 / v2
