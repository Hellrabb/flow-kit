...
- **R3.5 · 禁跳反问（弱模型鲁棒性 · 硬约束）** 需反问的阶段（`0-change` / `1-requirement` / `2-design`），反问 gate 未完成（用户未回答澄清问题）前，**禁止**产出最终方案或进入实现。弱模型最易跳过反问直接写代码——此为硬约束非建议。详见各阶段 prompt 的「反问 gate」段。
...
- **R6.1 · 引用前证据链（弱模型鲁棒性 · 硬约束）** 引用任何**文件 / 外部库 / API / 字段名 / 既有抽象**前，**必须**先在代码库或文档里 `grep`/`read` 验证其存在。未验证的引用必须显式标注「未找到，拒绝引用」或停下来确认——**禁止凭空假设/凭印象引用**（弱模型最高频幻觉源）。此规则即 L3 证据链机制，详见 `@flow-kit/prompts/4-dev.md` / `2-design.md`。
...
- **R7.4 · 动手前复述边界（弱模型鲁棒性 · 防 scope drift）** 4-dev 执行任务前，必须先复述当前 task 的 `read_files` / `write_files` 边界 + CHANGE「范围排除」段。弱模型最易把「本次不做」当成要做（范围漂移）——复述边界是防漂移的硬 gate，非可选。
