...
# ── Gate 3: 阶段 ∈ {1,2,6} 且独立 review gate 开启 ──
[[ "$phase" =~ ^(1|2|6)$ ]] || exit 0
...
if [ -n "$content" ]; then
  ...
  {
    echo "# 独立审查 · 阶段 ${phase}（L3 · ${model}）"
    echo ""
    echo "> 自动生成于 ${ts}。由独立外部模型盲审，未经主 agent。"
    echo ""
    echo "## 审查结论"
    echo ""
    echo '```json'
    ...
    echo '```'
    echo ""
    echo "<details><summary>API 原始响应（折叠）</summary>"
    echo ""
    echo '```json'
    echo "$ai_response"
    echo '```'
  ...
  jq -n --arg p "$phase" --arg ts "$ts_iso" --arg rf "INDEPENDENT-REVIEW-${phase}.md" \
    '{phase:$p, status:"done", fail_count:0, written_at:$ts, report_file:$rf}' \
