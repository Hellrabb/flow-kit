...
scan_l1_rules() {
  ...
  if [[ -n "$forbidden_paths" ]]; then
    ...
    while IFS= read -r fpath; do
      [[ -z "$fpath" ]] && continue
      if echo "$all_touched" | grep -qF "$fpath" 2>/dev/null; then
...
scan_l3_evidence() {
  ...
  while IFS= read -r cpath; do
    ...
    # Check if the cited path exists in tool call evidence
    # Use fuzzy match: the cited path should appear as a substring in evidence
    # or evidence should contain the cited path
    local found=false
    if echo "$all_evidence" | grep -qF "$cpath" 2>/dev/null; then
      found=true
    fi

    # Also check if the path exists on disk (model may have cited a real path
    # from prior knowledge — this is still a violation if not in tool calls)
    # Skip on-disk check for paths starting with ~ (home dir refs)
