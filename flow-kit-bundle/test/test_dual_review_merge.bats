...
setup() {
  ...
  # 路径
  L3_LIB="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/flow-kit-bundle/hooks/stop/lib/l3-review.sh"
  DONE_VAL_LIB="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/flow-kit-bundle/hooks/stop/lib/done-validation.sh"
...
@test "AC-1: 29-independent-review.sh contains L2-wait gating for both mode" {
  ...
  hook29="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/flow-kit-bundle/hooks/stop/29-independent-review.sh"
...
@test "AC-1: independent-review-gate.sh contains L2-wait gating for PreToolUse path" {
  gate_script="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/flow-kit-bundle/hooks/pre-tool-use/independent-review-gate.sh"
...
@test "AC-2: L2-blind-review.md contains append-first constraints" {
  l2_blind="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/flow-kit-bundle/flow-kit/prompts/independent/L2-blind-review.md"
...
@test "AC-6: all 6 phase prompts contain L3_verdict=skipped for L2-only" {
  prompts_dir="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/flow-kit-bundle/flow-kit/prompts"
...
@test "AC-5: done-validation.sh phase_name mapping covers {1,2,3,5,6,7}" {
  done_val="$(cd "$(dirname "$BATS_TEST_FILENAME")/.." && pwd)/flow-kit-bundle/hooks/stop/lib/done-validation.sh"
