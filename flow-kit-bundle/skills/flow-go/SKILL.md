---
name: flow-go
description: flow-kit 统一入口 — 自动路由到对应阶段（新需求/继续/执行任务/审查/测试/集成）
---
...
| 用户输入特征 | 路由到 | 备注 |
|---|---|---|
| `继续` / `接着上次` / `恢复` / `resume` | `prompts/4-dev.md` 的「入场恢复」段 | 加载 `.flow-active` 的 `interrupt` 上下文（active_file + last_action + checkpoint_at）注入恢复提示 |
