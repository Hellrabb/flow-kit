...
# v1 立场：✅ 挡（T2 KVP 校验 + Tier 2 T3 握手锚点 + D7 _is_dotdone_write 拦 Write tool）
# 依赖：fk_validate_done_marker T2/T3（T06）+ _is_dotdone_write（T07）
set -uo pipefail
...
if ! grep -qE 'fk_validate_done_marker[[:space:]]*\(\)' "$ARTIFACTS_LIB" 2>/dev/null || \
  ...
  echo "⏳ PENDING: fk_validate_done_marker（T06）/ _is_dotdone_write（T07）未实现 — 威胁②③ 载体就位，待激活"
  exit 0
fi

# setup 威胁场景：
# ② 伪造 KVP（written_by 谎称 review-subagent）
# ③ 主 agent 自产，无 .independent-review-6.done 真实握手（29 号子进程未跑）
...
L3_verdict=pass
EOF
# 注：无 .independent-review-6.done 握手文件 → T3 握手锚点缺失
