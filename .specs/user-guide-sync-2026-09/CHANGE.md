...
- 面向用户的 `FLOW-KIT-用户指南.md` 停在 **20260713**，`flow-kit-用户指南.pptx` 停在 2026-07-31 快照：仍写「17 模块 Stop Hook」「L2/L3 三级优先级链」「仅 Claude Code」、无 dsh 平台与插件安装章节、无 `/flow doctor` 与新门禁描述——按指南操作会得到与 2026-09 实现不一致的行为，误导新老用户。
...
- PPTX 重生成：沿用 `.specs/archive/2026-07-31-user-guide-ppt-sync/gen/` 生成器（slides.json + masters.py + 图表源），更新过时 slide 内容并（如版面允许）新增 dsh 插件页；日期更新为 2026-09-03。
...
- `flow-kit-用户指南.pptx` 由生成器重建成功、日期为 2026-09-03、关键 slide 与 MD 一致、渲染验证无版式/缺字问题；根目录与 bundle 的 MD 副本一致。
...
- 指南 ~70KB、PPT 19 页，增量修订点多，存在漏改/事实漂移风险 → TEST 阶段用清单化 grep 断言兜底。
