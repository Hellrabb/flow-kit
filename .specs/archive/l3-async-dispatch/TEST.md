...
| **L2-gap** | L2-only L2未完成时派发提示+拒绝skip (4f7f81a) | ✅ pass |

## 回归测试

| 测试集 | 结果 |
|---|---|
| 全量 bats（790 tests） | 620 pass / 170 预现有失败（路径 bug，非本次改动引入） |
| L3 相关现有测试（l3_feedback / l3_review / dual_review_merge / l2_l3_fix_compliance / l2_l3_granular） | ✅ 全绿（预现有失败同上原因） |
