...
- **NFR 验证口径**: 性能「≤120s/≤90KB」为记录值——TEST 阶段在 TEST.md 记录实测（超 600s 判 fail）；安全自动断言 = 检索范围内无 `sk-sp-` 前缀与绝对 `/home/hellrabbit` 路径（grep 命中数 0）；可访问性/兼容性为人工检查清单（deck 字号常量 diff、三平台表述一致性，见 REVIEW 检查项），明示不纳入自动 gate
