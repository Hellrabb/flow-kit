...
| TD-024 | 🟡 | `test/test_l3_pipeline_fix.bats:440-441` | 全量 bats 偶发 1 fail（2026-09-06 全量扫描 F-1 · T04 AC-1 · status 141 SIGPIPE）：`printf '%s\n' "$out" | grep -qF` 在 grep 命中即退 + pipefail 下 writer 收 SIGPIPE → 管道 141；首轮全量 1 not-ok，复跑 803/803 绿、单文件 41/41 绿 → 满负荷偶发 flaky，门禁可复现性隐患 | 断言改 `[[ "$out" == *marker* ]]` 或 `grep -F … >/dev/null`（去 -q）——一行修，随下个 health-fix change | `M-health 2026-09-06 全量扫描` |
