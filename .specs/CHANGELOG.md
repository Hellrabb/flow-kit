...
| 2026-09-02 | dsh-flow-kit-sync-2026-09 | flow-kit dsh 插件同步 2026-09 flow-kit 更新：correction 卫生/状态守卫（ADR-024）+ shellcheck 清零（TD-023）+ install.sh dsh 平台分支 + L2/L3 站点级默认模型五级解析链（/flow model l2-default=/l3-default=/--clear + fk_resolve_model tier-4/5）· /flow doctor 新增 .flow-active.correction 卫生报告（type+violations 去重摘要）· v0.1.0→0.2.0 · 重打包 vendor 逐字节一致 + web/flowkit-test 两 profile 重装并确认挂载（commands+skills+systemPrompt）· node 单测 20/20 + bats 770/770 + make lint + check-test-sync · 降挡（无 L2/L3：纯内容同步 + 运行环境无 FLOW_KIT_L3_* 凭证；7-integration=off，理由记录于 CHANGE.md「独立审查」段，可随时 /flow l2-review 7 补审） | - |
