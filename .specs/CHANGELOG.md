...
| 2026-09-05 | `l3-prompt-loop-fix` | INDEPENDENT-REVIEW-7（chisel_env traceweave）3 条 flow-kit 缺陷核查→修 2 真 1 误判：L3 prompt 反馈优先重构（前轮发现单行摘要 severity\|file\|issue ≤600B + (+k more) 折叠 + 响应要点 ≤200B + 四象限注入矩阵 + 缺失/空静默边界）+ UTF-8 字节截断双 helper（_l3_utf8_head_bytes/stream · 13 位点全替换）+ 归档布局 dirname×3 修正 + checklist 对齐归档约定 + jq 模板指令前置（总输出封顶不切 JSON 契约）· Claim 3（jq empty fail-open）核查为文档化设计不修 · T01-T06 串行 TDD · 803 bats 0 fail · 五副本 md5 唯一 · L2×6 轮 + L3×6 轮全 pass（含 L2 R1 字节切片非法 UTF-8 等 fix-loop 修复） | L-085, L-086, L-087, L-088 |
| 2026-09-03 | user-guide-sync-2026-09 | 用户指南与用户指南 PPT 同步至 2026-09 功能集（内容同步型 · 纯文档/演示）：指南版本 20260713→2026-09-03、§1 平台化（CC/dsh/OC + dsh-flow-kit 插件条目）、§2.4 dsh 插件安装、§4 /flow model 五级链与 doctor correction 报告、gate-config 值域 both/L2/L3、§7 Stop Hook 12 开关模块（config 键列）+ 33/34 号 + pre-commit + ADR-024 卫生口径、§12 索引；新建 deck 生成器 .specs/user-guide-deck-gen/（slides.json 20 页 + deck_checks）重建 flow-kit-用户指南.pptx 19→20 页并渲染验证；bundle 副本逐字节同步 · 全阶段 1/2/3/5/6/7 L2+L3 独立审查闭环（17+ 轮 findings 修复，Minor 登记 M1-M19）· 每笔提交 pre-commit 770 bats 0 fail | L-083, L-084 |
| 2026-09-03 | dsh-flow-kit-sync-2026-09 | flow-kit dsh 插件同步 2026-09 flow-kit 更新：correction 卫生/状态守卫（ADR-024）+ shellcheck 清零（TD-023）+ install.sh dsh 平台分支 + L2/L3 站点级默认模型五级链（/flow model l2-default=/l3-default=/--clear + fk_resolve_model tier-4/5）· /flow doctor 增 .flow-active.correction 卫生报告 · v0.1.0→0.2.0 · 重打包 vendor 逐字节一致 + web/flowkit-test 两 profile 重装挂载 · node 20/20 + bats 770/770 + make lint/test-sync · 降挡（9-02）→ 用户配置 FLOW_KIT_L3_* 后补审（9-03）：L2 盲审 pass（R1 /flow model 回显修复等 3 findings 闭环）+ L3 首审 fail → 归档六件套补齐 + CHANGELOG 归位 + L-082 后重审 pass → 全阶段扩展：gate_config=all，阶段 1/2/3/5/6 补 L2 盲审（17 findings 闭环：doctor check→rule 回退等代码修复 + 各阶段文档加固）+ L3 外部审查（phase 3 三轮迭代后全过），六阶段 done 锚点 L2/L3 双 pass | L-082 |
...
| 2026-08-08 | l2l3-cross-platform | L2/L3 模型审查双平台兼容性彻底修复：平台翻转凭证优先级（opencode Path3>P1>P2 / CC Path1>P3>P2）+ transcript-parser 工具名双形状（CC tool_use+Agent / opencode tool+task）+ AC-6 bats 红线断言 + 共享函数 fk_resolve_api_credentials + 平台检测 fk_platform_is_opencode + opencode reviewer agent 定义 + install_hooks.sh agent 安装段 + bats 752/752 + make check 四门 | L-078,L-079 |
| 2026-09-01 | correction-hygiene-state-guard | correction 文件卫生 + 外来状态守卫：correction_file_dedupe/trim/strip_type 共享函数（9 check 白名单·ADR-024）+ 33 号健康清零/去重/容量 10 FIFO + 外来（YAML）让位（清陈旧+剥 type+去重 note·AC-7 只读）+ 29 号 l2-missing 写入 compliance 保护 + 退场机制（M0 块·Gate3 前置）+ Makefile check-validate 权威行 + bats 764/764 + make check 四门 + chisel_env 真实回放 50→1 | L-080 |
